...
public class SMTIBEA_BitFlipMutation extends Mutation {
  /**
  ...
  private Double mutationProbability_ = null ;
	 ...
	 */
	public SMTIBEA_BitFlipMutation(HashMap<String, Object> parameters) {
		super(parameters) ;
	...
	public void doMutation(double probability, Solution solution) throws JMException {
		try {
			if ((solution.getType().getClass() == BinarySolutionType.class) ||
					(solution.getType().getClass() == BinaryRealSolutionType.class) || solution.getType().getClass() == SMTIBEA_BinarySolution.class) {
				for (int i = 0; i < solution.getDecisionVariables().length; i++) {
					//for (int j = 0; j < ((Binary) solution.getDecisionVariables()[i]).getNumberOfBits(); j++) {
                                        for (Integer j : SMTIBEA_Problem.featureIndicesAllowedFlip){ //flip only not "fixed" features
